...
export async function generatePdf(writeup: WriteupData, username: string) {
  ...
  function text(str: string, x: number, size: number, rgb: [number,number,number], style: 'normal'|'bold' = 'normal', maxW?: number) {
  ...
  function codeBlock(label: string, content: string, rgb: [number,number,number]) {
    ...
    pdf.rect(ML, y + 4, CW, 4, 'F'); // flatten bottom of header

    // Traffic lights
    pdf.setFillColor(255, 95, 87); pdf.circle(ML + 5, y + 4, 1.2, 'F');
    pdf.setFillColor(254, 188, 46); pdf.circle(ML + 10, y + 4, 1.2, 'F');
    pdf.setFillColor(40, 200, 64);  pdf.circle(ML + 15, y + 4, 1.2, 'F');
